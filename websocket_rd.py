...
# Read stream
async def input_async_worker(websocket, path):
    ...
    while(True):
        ...
        # Parse params
        packet_type = decode_int8(msg[0:1])
        payload = msg[1:]
...
async def view_async_worker(websocket, path):
...
async def main():
    ...
    viewSocketServer = await websockets.serve(view_async_worker, interface_ip, args.view_port)
    inputSocketServer = await websockets.serve(input_async_worker, interface_ip, args.input_port)

    await asyncio.gather(viewSocketServer.wait_closed(), inputSocketServer.wait_closed())

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument('--view-port', type=int, default=8765, metavar='{1..65535}', choices=range(1, 65535), help='view port')
    parser.add_argument('--input-port', type=int, default=8766, metavar='{1..65535}', choices=range(1, 65535), help='input port')
    args = parser.parse_args()

    if(args.view_port == args.input_port):
        print("view-port cannot equal input-port")
        exit(1)
